...
class InteractiveTool():
    ...
    def __init__(self,
        ...
        file_chooser_controls = [self.file_chooser_line, self.load_line_button, self.file_chooser_aperture, self.load_aperture_button, self.file_chooser_optics, self.load_optics_button]
        first_row_controls = [self.knob_dropdown, self.add_button, self.remove_button, self.apply_button, self.reset_button]
        second_row_controls = [self.cycle_input, self.cycle_button, self.envelope_input, self.envelope_button, self.plane_dropdown, self.plane_button]
        self.widgets = file_chooser_controls+first_row_controls+second_row_controls+second_row_controls     
        timber_row_controls = self.initialise_timber_data(spark) # If spark was not given this will return a None  

        # Put all the widgets together into a nice layout
        self.define_widget_layout(file_chooser_controls, first_row_controls, second_row_controls, timber_row_controls)
    ...
    def create_file_choosers(self):
        ...
        self.load_line_button = Button(description="Load line", style=widgets.ButtonStyle(button_color='pink'))
        self.load_aperture_button = Button(description="Load aperture", style=widgets.ButtonStyle(button_color='pink'))
        self.load_optics_button = Button(description="Load optics", style=widgets.ButtonStyle(button_color='pink'))
    ...
    def initialise_timber_data(self, spark):
        
        # Only add timber buttons if spark given as an argument
        if spark:

            self.collimator_data = CollimatorsData(spark)
            self.BPM_data = BPMData(spark)
            
            self.load_BPMs_button = Button(description="Load BPMs", style=widgets.ButtonStyle(button_color='pink'))
            self.load_BPMs_button.on_click(self.on_load_BPMs_button_clicked)

            self.load_cols_button = Button(description="Load collimators", style=widgets.ButtonStyle(button_color='pink'))
    ...
    def define_widget_layout(self,
                             file_chooser_controls: List[widgets.Widget], 
                             first_row_controls: List[widgets.Widget], 
                             second_row_controls: List[widgets.Widget],
                             timber_row_controls: List[widgets.Widget]) -> VBox:
        """
    ...
    def create_buttons(self):
        ...
        self.add_button = Button(description="Add", style=widgets.ButtonStyle(button_color='rgb(179, 222, 105)'))
        # Button to remove selection
        self.remove_button = Button(description="Remove", style=widgets.ButtonStyle(button_color='rgb(249, 123, 114)'))
        # Button to apply selection and update graph
        self.apply_button = Button(description="Apply", style=widgets.ButtonStyle(button_color='pink'))
        # Button to shift the graph
        self.cycle_button = Button(description="Cycle", style=widgets.ButtonStyle(button_color='pink'))
        # Button to reset knobs    
        self.reset_button = Button(description="Reset knobs", style=widgets.ButtonStyle(button_color='rgb(255, 242, 174)'))
        # Button to change envelope size# Create a dropdown to select a knob
        self.envelope_button = Button(description="Apply", style=widgets.ButtonStyle(button_color='pink'))
        self.plane_button = Button(description="Switch", style=widgets.ButtonStyle(button_color='pink'))
